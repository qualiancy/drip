...
Drip.prototype.on = function (ev, fn) {
  var events = ev.split(this.delimeter)
  ...
  while (events.length) {
    ...
    if (events.length === 0) {
      (callbacks[event]._ = callbacks[event]._ || []).push(fn);
    } else {
...
Drip.prototype.off = function (ev, fn) {
  ...
  var traverse = function (events, map) {
    ...
    if (map['*'] && map['*']._.length) clean('*', map['*']._);
    if (events.length) {
      map = map[event];
      traverse(events, map);
    }
...
Drip.prototype.emit = function (ev) {
  ...
  var traverse = function (events, map) {
    ...
    if (map[event] && map[event] && !events.length) execute(map[event]._);
    if (map['*'] && map['*']._ && !events.length) execute(map['*']._);
    
    if (events.length) {
      if (map[event]) traverse(events, map[event]);
      if (map['*']) traverse(events, map['*']);
    }
  };
